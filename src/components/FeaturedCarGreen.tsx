...
export function FeaturedCarGreen() {
  ...
  const goToNext = () => {
    setCurrentIndex((prev) => (prev + 1) % carExtraExtensions.length);
  };

  const goToPrev = () => {
    setCurrentIndex(
      (prev) => (prev - 1 + carExtraExtensions.length) % carExtraExtensions.length
    );
  };
  ...
  return (
    <section className="relative h-screen overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 h-full flex items-start pt-[clamp(1.5rem,4vh,5rem)]">
